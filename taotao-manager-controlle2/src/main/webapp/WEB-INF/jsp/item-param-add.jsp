...
<script style="text/javascript">
	$(function(){
		...
		$("#itemParamAddTable .submit").click(function(){
			var params = [];//定义个数组，来收集一级分类
			var groups = $("#itemParamAddTable [name=group]");//找到所有的规格组
			groups.each(function(i,e){//遍历group 一级分类
				var p = $(e).parentsUntil("ul").parent().find("[name=param]");//遍历二级规格
				var _ps = [];//把所有二级规格放在这里
				p.each(function(_i,_e){
					var _val = $(_e).siblings("input").val();
					if($.trim(_val).length>0){//判断是否输入二级分类
						_ps.push(_val);	//放到数组里					
					}
				});
				var _val = $(e).siblings("input").val();
				if($.trim(_val).length>0 && _ps.length > 0){//一级内容不为空，并且它的二级规格有值
					params.push({
						"group":_val,
						"params":_ps
					});					
			...
			$.post(url,{"paramData":JSON.stringify(params)},function(data){//JSON.stringify 将json对象转成字符串
				if(data.status == 200){
